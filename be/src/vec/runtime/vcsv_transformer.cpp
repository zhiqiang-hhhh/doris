    ...
    for (size_t i = 0; i < block.rows(); i++) {
        for (size_t col_id = 0; col_id < block.columns(); col_id++) {
            ...
            if (col.column->is_null_at(i)) {
                fmt::format_to(std::back_inserter(_outstream_buffer), "{}", NULL_IN_CSV);
            } else {
                switch (_output_vexpr_ctxs[col_id]->root()->type().type) {
                case TYPE_BOOLEAN:
                case TYPE_TINYINT:
                    fmt::format_to(
                            std::back_inserter(_outstream_buffer), "{}",
                ...
                case TYPE_FLOAT: {
                    ...
                    fmt::format_to(std::back_inserter(_outstream_buffer), "{}", buffer);
                    break;
                }
                case TYPE_DOUBLE: {
                ...
                case TYPE_DATEV2: {
                    ...
                    fmt::format_to(std::back_inserter(_outstream_buffer), "{}", buf);
                    break;
                }
                ...
                case TYPE_STRING: {
                    ...
                    fmt::format_to(std::back_inserter(_outstream_buffer), "{}", value.to_string());
                    break;
                }
                case TYPE_DECIMALV2: {
                    const DecimalV2Value decimal_val(
                            reinterpret_cast<const PackedInt128*>(col.column->get_data_at(i).data)
                                    ->value);
                    fmt::format_to(std::back_inserter(_outstream_buffer), "{}", decimal_val.to_string());
                    break;
                }
                case TYPE_DECIMAL32: {
                    fmt::format_to(std::back_inserter(_outstream_buffer), "{}", col.type->to_string(*col.column, i));
                    break;
                }
                case TYPE_DECIMAL64: {
                    fmt::format_to(std::back_inserter(_outstream_buffer), "{}", col.type->to_string(*col.column, i));
            ...
            if (col_id < block.columns() - 1) {
                fmt::format_to(std::back_inserter(_outstream_buffer), "{}", _column_separator);
            }
        }
        fmt::format_to(std::back_inserter(_outstream_buffer), "{}", _line_delimiter);
    }
