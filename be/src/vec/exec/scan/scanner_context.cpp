...
Status ScannerContext::init() {
    ...
    // _max_concurrency controls how many scanners of this ScanOperator can be submitted to scheduler at most.
    ...
    RETURN_IF_ERROR(schedule_scan_task(nullptr, l));
...
Status ScannerContext::get_block_from_queue(RuntimeState* state, vectorized::Block* block,
    ...
    if (scan_task != nullptr) {
        ...
        if (scan_task->cached_blocks.empty()) {
            ...
            RETURN_IF_ERROR(schedule_scan_task(scan_task, l));
        }
    }
...
int32_t ScannerContext::_get_margin(std::unique_lock<std::mutex>& transfer_lock) {
    ...
    //     margin = margin_1;
    // } else {
    //     // 到这里说明 _scan_scheduler 的资源使用没有饱和，我们可以让当前的 scan operator 多提交一些 scan task
...
std::shared_ptr<ScanTask> ScannerContext::pull_next_scan_task(
