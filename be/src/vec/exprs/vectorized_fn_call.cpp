...
void VectorizedFnCall::prepare_ann_range_search(
    ...
    if (!array_expr->is_constant()) {
        suitable_for_ann_index = false;
        return;
    }
    if (!array_expr->get_const_col(nullptr, &column_wrapper).ok()) {
        suitable_for_ann_index = false;
        return;
    }
    ...
    if (auto* nullable_top = check_and_get_column<ColumnNullable>(*top_col)) {
        if (nullable_top->has_null()) {
            suitable_for_ann_index = false; // query vector cannot be NULL
            return;
        }
    ...
    if (value_count == 0) {
        suitable_for_ann_index = false; // empty query vector is invalid
        return;
    }
    if (auto* values_nullable = check_and_get_column<ColumnNullable>(nested_data_any)) {
        if (values_nullable->has_null(0, value_count)) {
            suitable_for_ann_index = false; // elements cannot contain NULL
            return;
        }
    ...
    for (size_t i = 0; i < value_count; ++i) {
        range_search_runtime.query_value[i] = data_ref[i];
    }
