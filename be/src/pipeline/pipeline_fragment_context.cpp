...
Status PipelineFragmentContext::_build_pipeline_tasks(
    ...
    _total_tasks = 0;
    LOG_INFO("Instance {} has {} pipelines.", print_id(_fragment_instance_id), _pipelines.size());
...
Status PipelineFragmentContext::_build_operators_for_set_operation_node(ExecNode* node,
                                                                        PipelinePtr cur_pipeline) {
    // A typical set ExecNode will have three pipelines.
    // One as data sink, two as data source of set operator.
    auto sink_pipeline = add_pipeline();
    ...
    for (int child_idx = 1; child_idx < child_size; ++child_idx) {
        ...
        RETURN_IF_ERROR(_build_pipelines(node->child(child_idx), probe_pipeline));
        OperatorBuilderPtr probe_sink_builder =
                std::make_shared<SetProbeSinkOperatorBuilder<is_intersect>>(node->id(), child_idx,
                                                                            node);
    ...
    return cur_pipeline->add_operator(source_builder);
}
