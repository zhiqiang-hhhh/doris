...
void PartitionedHashJoinProbeLocalState::update_profile_from_inner() {
    ...
    if (_shared_state->inner_runtime_state) {
        ...
        if (_shared_state->need_to_spill) {
            update_build_profile<true>(sink_local_state->custom_profile());
            update_probe_profile<true>(probe_local_state->custom_profile());
        } else {
            update_build_profile<false>(sink_local_state->custom_profile());
            update_probe_profile<false>(probe_local_state->custom_profile());
        }
    }
}
