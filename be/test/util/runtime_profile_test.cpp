...
TEST(CountersTest, Basic) {
    ...
    RuntimeProfile profile_a2("ProfileAb");

    TRuntimeProfileTree thrift_profile;

    profile_a.add_child(&profile_a1, false);
    profile_a.add_child(&profile_a2, false);
...
TEST(CountersTest, MergeAndupdate) {
    ...
    profile1.add_child(&p1_child1, false);
    profile1.add_child(&p1_child2, false);

    RuntimeProfile profile2("Parent2");
    RuntimeProfile p2_child1("Child1");
    RuntimeProfile p2_child3("Child3");
    profile2.add_child(&p2_child1, false);
    profile2.add_child(&p2_child3, false);

    // Create parent level counters
    RuntimeProfile::Counter* parent1_shared =
            profile1.add_counter("Parent Shared", TUnit::UNIT);
    RuntimeProfile::Counter* parent2_shared =
            profile2.add_counter("Parent Shared", TUnit::UNIT);
    RuntimeProfile::Counter* parent1_only =
            profile1.add_counter("Parent 1 Only", TUnit::UNIT);
    RuntimeProfile::Counter* parent2_only =
            profile2.add_counter("Parent 2 Only", TUnit::UNIT);
    ...
    RuntimeProfile::Counter* p1_c1_shared =
            p1_child1.add_counter("Child1 Shared", TUnit::UNIT);
    ...
    RuntimeProfile::Counter* p2_c1_shared =
            p2_child1.add_counter("Child1 Shared", TUnit::UNIT);
    ...
    // Doris does not copy constructor, so we create a same one.
    RuntimeProfile merged_profile("Parent1");
    profile1.add_child(&p1_child1, false);
    profile1.add_child(&p1_child2, false);
...
TEST(CountersTest, DerivedCounters) {
    ...
    RuntimeProfile::DerivedCounter* throughput_counter = profile.add_derived_counter(
            ...
            [bytes_counter, ticks_counter] { return RuntimeProfile::units_per_second(bytes_counter, ticks_counter); }, "");
...
TEST(CountersTest, RateCounters) {
    ...
    RuntimeProfile::Counter* rate_counter = profile.add_rate_counter("RateCounter", bytes_counter);
    EXPECT_TRUE(rate_counter->type() == TUnit::BYTES_PER_SECOND);
