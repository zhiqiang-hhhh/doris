...
protected:
    void SetUp() override {
        ...
        _index_file_writer = std::make_unique<MockIndexFileWriter>(doris::io::global_local_filesystem());
    }
...
TEST_F(AnnIndexWriterTest, TestConstructorAndDestructor) {
    // Test constructor
    auto writer = std::make_unique<AnnIndexColumnWriter>(_index_file_writer.get(), _tablet_index.get());
...
TEST_F(AnnIndexWriterTest, TestInitWithDifferentProperties) {
    ...
    std::vector<std::map<std::string, std::string>> test_properties = {
        {{"index_type", "hnsw"}, {"metric_type", "inner_product"}, {"dim", "8"}, {"max_degree", "32"}},
        {{"index_type", "hnsw"}, {"metric_type", "l2_distance"}, {"dim", "128"}, {"max_degree", "64"}},
    ...
    for (const auto& props : test_properties) {
        ...
        auto writer = std::make_unique<AnnIndexColumnWriter>(_index_file_writer.get(), tablet_index.get());
        
        auto fs_dir = std::make_shared<DorisFSDirectory>();
        EXPECT_CALL(*_index_file_writer, open(testing::_))
            .WillOnce(testing::Return(fs_dir));
