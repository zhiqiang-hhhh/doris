...
// Additional test cases to cover early returns in VectorizedFnCall::prepare_ann_range_search
TEST_F(VectorSearchTest, TestPrepareAnnRangeSearch_EarlyReturn_NullRoot) {
    // Case 1: VExprContext with null _root
    doris::VectorSearchUserParams user_params;
    auto ctx = std::make_shared<VExprContext>();
    // _root is null by default, this should return OK early
    auto status = ctx->prepare_ann_range_search(user_params);
    EXPECT_TRUE(status.ok());
    // ann_range_search should remain false since it was not set
...
TEST_F(VectorSearchTest, TestPrepareAnnRangeSearch_EarlyReturn_WrongOpcode) {
    ...
    VExpr::create_expr_tree(texpr, ctx);
    ASSERT_TRUE(ctx->prepare(state.get(), row_desc).ok());
...
TEST_F(VectorSearchTest, TestPrepareAnnRangeSearch_EarlyReturn_NonLiteralRight) {
    // Case 3: Right child is not a literal - create an expression with two slot refs
    TExpr texpr;
    texpr.nodes.resize(3);

    // Root node - comparison operation
    texpr.nodes[0].node_type = TExprNodeType::FUNCTION_CALL;
    texpr.nodes[0].opcode = TExprOpcode::GE;
    texpr.nodes[0].num_children = 2;
    texpr.nodes[0].fn.name.function_name = "ge";
    texpr.nodes[0].fn.signature = "ge(double, double)";

    // Left child - virtual slot ref (distance)
    texpr.nodes[1].node_type = TExprNodeType::SLOT_REF;
    texpr.nodes[1].num_children = 0;
    texpr.nodes[1].slot_ref.slot_id = 3;
    texpr.nodes[1].slot_ref.tuple_id = 0;
    texpr.nodes[1].slot_ref.is_virtual_slot = true;

    // Right child - another slot ref instead of literal
    texpr.nodes[2].node_type = TExprNodeType::SLOT_REF;
    texpr.nodes[2].num_children = 0;
    texpr.nodes[2].slot_ref.slot_id = 0; // Different slot
    texpr.nodes[2].slot_ref.tuple_id = 0;
    texpr.nodes[2].slot_ref.is_virtual_slot = false;
