...
protected:
    void SetUp() override {
        ...
        _ann_reader = std::make_shared<AnnIndexReader>(_tablet_index.get(), _mock_index_file_reader);
    }

    void TearDown() override {
        doris::vectorized::VectorSearchTest::TearDown();
    }
...
TEST_F(AnnIndexIteratorTest, TestReadFromIndexWithNullParam) {
    ...
    IndexParam param = static_cast<doris::vectorized::AnnIndexParam*>(nullptr);
...
TEST_F(AnnIndexIteratorTest, TestReadFromIndexWithValidParam) {
    ...
    doris_faiss_vector_index->set_metric(Metric::L2);
...
TEST_F(AnnIndexIteratorTest, TestWithInnerProductMetric) {
    ...
    auto doris_faiss_vector_index = std::make_unique<FaissVectorIndex>();
    doris_faiss_vector_index->set_metric(Metric::IP);
