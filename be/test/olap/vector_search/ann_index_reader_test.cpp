...
TEST_F(AnnIndexReaderTest, TestConstructor) {
    auto reader = std::make_unique<segment_v2::AnnIndexReader>(_tablet_index.get(), _mock_index_file_reader);
    
    EXPECT_NE(reader, nullptr);
    EXPECT_EQ(reader->get_index_id(), 1);
    EXPECT_EQ(reader->index_type(), IndexType::ANN);
    EXPECT_EQ(reader->get_metric_type(), segment_v2::Metric::L2);
}
...
TEST_F(AnnIndexReaderTest, TestConstructorWithDifferentMetrics) {
    ...
    auto reader = std::make_unique<segment_v2::AnnIndexReader>(tablet_index.get(), _mock_index_file_reader);
    
    EXPECT_EQ(reader->get_metric_type(), segment_v2::Metric::IP);
...
TEST_F(AnnIndexReaderTest, TestLoadIndexSuccess) {
    ...
    ON_CALL(*_mock_index_file_reader, open(testing::_, testing::_))
        .WillByDefault(testing::Invoke([](const doris::TabletIndex*, const doris::io::IOContext*) 
            -> doris::Result<std::unique_ptr<doris::segment_v2::DorisCompoundReader, doris::segment_v2::DirectoryDeleter>> {
...
TEST_F(AnnIndexReaderTest, TestQueryWithoutLoadIndex) {
    ...
    vectorized::AnnIndexParam param {
...
TEST_F(AnnIndexReaderTest, TestRangeSearchWithoutLoadIndex) {
    ...
    vectorized::RangeSearchParams params;
...
TEST_F(AnnIndexReaderTest, TestWithInnerProductMetric) {
    ...
    auto doris_faiss_vector_index = std::make_unique<doris::segment_v2::FaissVectorIndex>();
    doris_faiss_vector_index->set_metric(doris::segment_v2::Metric::IP);
