...
TEST_F(AnnIndexEdgeCaseTest, TestAnnIndexWriterWithEmptyProperties) {
    ...
    auto mock_file_writer = std::make_unique<MockIndexFileWriter>(doris::io::global_local_filesystem());
    auto writer = std::make_unique<AnnIndexColumnWriter>(mock_file_writer.get(), tablet_index.get());
    
    auto fs_dir = std::make_shared<DorisFSDirectory>();
    EXPECT_CALL(*mock_file_writer, open(testing::_))
